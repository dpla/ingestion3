...
/** Produces QA reports related to thumbnails (i.e. oreAggregation.preview).
  ...
  * missing thumbnails. The resulting Dataframe has the following columns:
  ...
  * has to make an HTTP request for every thumbnail in the data sample. The
  ...
  * a preview URL The resulting DataFrame has the following columns:
  *   - localUri: String the provider URI for the item
  *   - previewUri: String the URI used for the thumbnail/preview in the DPLA
  *     portal
  *   - dplaUri: String, DPLA's URI for the item
  *
  * @param inputURI
  *   String, path to the data sample
  * @param outputURI
  *   String, path to write output
  * @param params
  *   Array[String], string value can be "missing" OR "dimensions"
  */
class ThumbnailReport(
  ...
  override def process(
    ...
    val token: String = getParams match {
      ...
      case _ =>
        throw new RuntimeException("No thumbnail report type specified.")
    }
  ...
  /** Get images with missing thumbnails. Images are items where
  ...
  def missingReport(
    ...
    val thumbnailData: Dataset[MissingThumbnail] = ds.map(dplaMapData => {
      ...
      val hasImageType =
        dplaMapData.sourceResource.`type`.exists(_.toLowerCase() == "image")
  ...
  def previewReport(
    ...
    thumbnailData
      .select("localUri", "dplaUri", "previewUri")
      .filter("hasPreview = TRUE")
  }

  /** Get thumbnail dimensions, and any errors that occur when attempting a
    * request the thumbnail endpoint. Warning: This process takes a long time to
    * run.
    */
  def dimensionsReport(
      ds: Dataset[OreAggregation],
      spark: SparkSession
  ): DataFrame = {
    import spark.implicits._
  ...
  /** @param oreAggregation
    * @return
    *   String, the DPLA URI for an item
    */
  ...
  /** @param oreAggregation
    * @return
    *   String, the provider URI for an item
    */
  ...
  /** Parse the height and width from the thumbnail image.
    ...
    * @return
    *   Dimensions case class containing the height and width, or an error
    ...
    bufferedImg(uri) match {
      case Success(bimg) =>
        ...
        if (bimg == null) Dimensions(error = Some("Buffered image is null."))
          ...
          Dimensions(height = Some(bimg.getHeight), width = Some(bimg.getWidth))
  ...
  /** Attempt to get the thumbnail image over HTTP.
    ...
    *   URI for the thumbnail
    * @return
    *   Success(BufferedImage) if attempt to read image over HTTP is successful.
...
case class ThumbnailDimensions(
    localUri: String,
    dplaUri: String,
