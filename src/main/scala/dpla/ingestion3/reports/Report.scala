...
/** Report: trait with common fields and methods for QA reports.
  ...
  * objects and stub their method calls, and create fixtures for the Dataset and
  * Dataframe. This could be grounds for future work.
  *
...
trait Report {
  ...
  /** Run the report, opening the input and output and invoking the process()
    ...
    *   Reporter.main()
    * @return
    *   Try object representing success or failure, for Reporter.main()
    */
  ...
  /** Process the incoming dataset (mapped or enriched records) and return a
    * DataFrame of computed results, typically values extracted from the records
    * and their counts.
    *
  ...
  /** Accepts Seq[Any] and tries to extract the String values which should be
    * included in the report.
    *
    * For fields like Title and Rights this is fairly straight forward but for
    * context case classes like EdmAgent and SkosConcept it will be slightly
    * trickier to report on multiple fields.
    *
    * If the sequence is empty then a String indicating that the value is
    * missing is returned.
    *
    * @param t
    *   Sequence to report on
    * @return
    */
  def extractValue(t: Seq[AnyRef]): Seq[String] = {
    if (t.isEmpty) {
      ...
      t.headOption.getOrElse("__MISSING No value to extract__") match {
        ...
        case _: DplaPlace =>
          t.map(
            _.asInstanceOf[DplaPlace].name
              .getOrElse("__MISSING DplaPlace.name__")
          )
        case _: DcmiTypeCollection =>
          t.map(
            _.asInstanceOf[DcmiTypeCollection].title
              .getOrElse("__MISSING DcmiTypeCollection.title__")
          )
        case _: EdmAgent =>
          t.map(
            _.asInstanceOf[EdmAgent].name.getOrElse("__MISSING EdmAgent.name__")
          )
        case _: EdmTimeSpan =>
          t.map(
            _.asInstanceOf[EdmTimeSpan].originalSourceDate
              .getOrElse("__MISSING EdmTimeSpan.originalSourceDate__")
          )
        case _: SkosConcept =>
          t.map(
            _.asInstanceOf[SkosConcept].providedLabel
              .getOrElse("__MISSING SkosConcept.providedLabel__")
          )
