...
/** Responsible for desugaring a DplaMapModel and converting it to a
  * Spark-native Row-based structure.
  *
  * PLEASE NOTE: The *ordering* of the parameters to each call to the Row()
  * constructor is significant, and cannot be changed without a concurrent
  * effort to rewrite all the data in the master dataset, and a matching edit to
  * ModelConverter which is responsible for creating sugared instances of
  * DplaMapData from the output of this code, and represents the opposite of
  * this transformation.
  */
...
object RowConverter {
  ...
  private[model] def fromDcmiTypeCollection(
      dcmiTypeCollection: DcmiTypeCollection
  ): Row = Row(
