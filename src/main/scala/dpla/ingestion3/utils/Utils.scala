...
object Utils {
  ...
  /**
    * Writes data in Dataset[Row] out to a CSV file or files (depending on the
    * number of partitions) with headers
    *
    * This method should replace writeLogsAsCsv
    *
    * @param path Path to write to
    * @param name Name of CSV file
    * @param df Data to write
    * @param partitions Number of partitions, default value is 1
    * @return List[String] Paths of the files successfully written
    */
  def writeCsv(path: String, name: String, df: Dataset[Row], partitions: Int = 1): Array[String] = {
    val tmpPath = File.createTempFile("i3-csv-tmp", "")

    df.coalesce(partitions)
      .write
      .mode(SaveMode.Overwrite)
      .option("header", "true")
      .csv(tmpPath.getAbsolutePath)

    // Drops the meta _SUCCESS, .crc .csc.crc files generated by Spark
    val tmpFiles = tmpPath.listFiles()
        .filter(file => file.getName.endsWith(".csv"))

    for(t <- tmpFiles.indices) {
      val csvFile = new File(s"$path/$name.csv")
      csvFile.getParentFile.mkdirs()
      tmpFiles(t).renameTo(csvFile)
    }
