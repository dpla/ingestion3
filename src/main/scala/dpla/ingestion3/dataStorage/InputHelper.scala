...
object InputHelper {
  ...
  /** Check if a given local or S3 path is a valid activity path. An activity is
    ...
    *
    * A valid path will match the following pattern: 8 digits (year month day)
    * underscore 6 digits (time) dash 0-n letters (provider short name) dash 0-n
    * characters (not "/") (schema) 0-1 "/"
    *
    * @param path
    *   Local or S3 path that may or may not be an activity.
    * @return
    *   True if the path matches the pattern for an activity.
    */
  def isActivityPath(path: String): Boolean = {
    val activityPath: Regex =
      """\d{8}_\d{6}-[A-Za-z0-9]*-[^/]*/?$""".r.unanchored
    path match {
  ...
  /** Get the most recent activity contained within a given local or S3 folder.
    ...
    *
    * @param path
    *   Path to a local or S3 folder.
    * @return
    *   Path to the most recent activity.
    */
  ...
  /** Sorts the contents of the given path to find the most recent folder.
    * within the provided path that is a valid activity path. The activity must
    * be the immediate child of the given folder.
    *
    * @return
    *   Option[String] Absolute path to the most recent activity.
    */
  ...
  private def mostRecentS3(address: S3Address): Option[String] = {
    ...
    val folder: String = lastKey
      .stripPrefix(address.prefix.getOrElse(""))
      .stripPrefix("/")
      .split("/")(0)
  ...
  /** Get the last key from a batch of S3 object listings.
    *
    * @param ol
    *   An S3 ObjectListing (i.e. a batch of objects)
    * @return
    *   The last listed key.
    */
