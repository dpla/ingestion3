...
object MessageProcessor {
  ...
  def getWarnings(ds: Dataset[Row]): Dataset[Row] =
    ds.select("message", "level", "field", "id", "value")
      .where(s"level=='${IngestLogLevel.warn}'")
      .distinct()
  ...
  def getMessageFieldSummary(
    ...
    val msgRptDs = d2.map({ case Row(msg: String, field: String, count: Long) =>
      MessageFieldRpt(msg, field, count)
    })

    val singleColDs = msgRptDs
      .select(
        concat(col("msg"), lit("|"), col("field"), lit("|"), col("count"))
      )
      .as("label")
    ...
    val topLine = msgFieldRptArray.map {
      case k: MessageFieldRpt =>
