...
class NhMapping extends XmlMapping with XmlExtractor
  with IngestMessageTemplates {

  // IdMinter methods
  override def useProviderName: Boolean = true

  override def getProviderName: String = "nh"
  ...
  override def provider(data: Document[NodeSeq]): ExactlyOne[EdmAgent] = EdmAgent(
    name = Some("New Hampshire"), // FIXME
    uri = Some(URI("http://dp.la/api/contributor/nh"))
  )
