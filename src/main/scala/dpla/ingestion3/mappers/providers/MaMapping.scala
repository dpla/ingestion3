...
class MaMapping
  ...
  /** <mods:name> <mods:role> <mods:roleTerm authority="marcrelator"
    * authorityURI="http://id.loc.gov/vocabulary/relators" type="text"
    * valueURI="http://id.loc.gov/vocabulary/relators/ctb">Contributor</mods:roleTerm>
    * </mods:role> <mods:namePart>American Photo-Relief Printing
    * Co</mods:namePart> </mods:name>
    */
  override def contributor(data: Document[NodeSeq]): ZeroToMany[EdmAgent] = {
    (getModsRoot(data) \ "name")
      .filter(n =>
        extractStrings(n \ "role" \ "roleTerm")
          .map(_.trim)
  ...
  def edmAgentUriHelper(node: Node): EdmAgent = {
    ...
    val scheme =
      getAttributeValue(node, "authorityURI").filter(_.nonEmpty).map(URI)
  ...
  private def nameConstructor(node: Node): Option[String] = {
    val name = extractStrings(
      (node \ "namePart").filter(p => p.attributes.isEmpty)
    ).mkString(". ")
  ...
  override def date(data: Document[NodeSeq]): Seq[EdmTimeSpan] = {
    ...
    props.flatMap(prop => {
      val dateCreated = (getModsRoot(data) \ "originInfo" \ prop)
        .filterNot(node =>
          filterAttribute(node, "point", "start") | filterAttribute(
            node,
            "point",
            "end"
          )
      ...
      (dateCreated.nonEmpty, earlyDate.nonEmpty, lateDate.nonEmpty) match {
        ...
        case (false, true, true) =>
          Seq(
            EdmTimeSpan(
              originalSourceDate = Some(
                s"${earlyDate.headOption.getOrElse("")}-${lateDate.headOption.getOrElse("")}"
              ),
  ...
  override def identifier(data: Document[NodeSeq]): Seq[String] =
    ...
    {
      val types =
        Seq("local-accession", "local-other", "local-call", "local-barcode")
  ...
  override def place(data: Document[NodeSeq]): Seq[DplaPlace] =
    ...
    // <mods:subject><mods:cartographics><mods:coordinates>
    {
      ...
      subjects.flatMap(subject => {
        ...
        val coordinates =
          extractString(subject \ "cartographics" \ "coordinates")

        val hierarchy = (subject \ "hierarchicalGeographic").map(h => {
          val cityLabel = (
            extractString(h \ "city"),
            extractString(h \ "citySection")
          ) match {
          ...
          val name = Option(
            Seq(cityLabel, county, state, country).flatten
              .map(_.trim)
              .mkString(", ")
          )
        ...
        if (places.isEmpty)
          ...
          Seq(DplaPlace(name = places, coordinates = coordinates))
      })
    }
