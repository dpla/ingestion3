...
class TnMapping
  ...
  override def collection(
    ...
    (data \\ "mods" \ "relatedItem")
      ...
      .flatMap(collection => {
        extractStrings(collection \ "titleInfo" \ "title")
          .map(Option(_))
          .zipAll(
            extractStrings(collection \ "abstract").map(Option(_)),
            None,
            None
          )
      })
      .map({ case (title: Option[String], desc: Option[String]) =>
        DcmiTypeCollection(title = title, description = desc, isShownAt = None)
      })
  ...
  override def place(data: Document[NodeSeq]): Seq[DplaPlace] =
    ...
    (data \\ "mods" \ "subject")
      .flatMap(place => {
        extractStrings(place \ "geographic")
          .map(Option(_))
          .zipAll(
            extractStrings(place \ "cartographics" \ "coordinates")
              .map(Option(_)),
            None,
            None
          )
      })
      .map({ case (name: Option[String], coord: Option[String]) =>
        DplaPlace(name = name, coordinates = coord)
      })
  ...
  override def relation(data: Document[NodeSeq]): ZeroToMany[LiteralOrUri] = {
    ...
    (data \\ "mods" \ "relatedItem")
      .filterNot(node =>
        (node \ "@type").text.equalsIgnoreCase("isReferencedBy")
      )
      .filterNot(node => (node \ "@type").text.equalsIgnoreCase("references"))
      .filterNot(node =>
        (node \ "@displayLabel").text.equalsIgnoreCase("project")
      )
      .flatMap(node =>
  ...
  override def rights(data: Document[NodeSeq]): AtLeastOne[String] =
    ...
    (data \\ "mods" \ "accessCondition")
      .flatMap(node =>
        getByAttribute(node, "type", "local rights statement")
      ) // TODO should this filter be in place?
      .flatMap(extractStrings)
  ...
  override def intermediateProvider(
    ...
    (data \\ "mods" \ "note")
      .flatMap(node =>
        getByAttribute(node, "displayLabel", "Intermediate Provider")
      )
      .flatMap(extractStrings)
      .map(nameOnlyAgent)
      .headOption
