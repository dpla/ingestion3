...
class CtMapping
  ...
  override def alternateTitle(data: Document[NodeSeq]): ZeroToMany[String] = {
    val altTitles = (data \ "titleInfo").flatMap(node =>
      getByAttribute(node, "type", "alternative")
    )
  ...
  override def place(data: Document[NodeSeq]): Seq[DplaPlace] =
    (extractStrings(
      data \ "subject" \ "cartographics" \ "coordinates"
    ) ++ // FIXME Shouldn't coordinates get mapped differently?
  ...
  override def title(data: Document[NodeSeq]): Seq[String] = {
    val altTitles = (data \ "titleInfo").filterNot(node =>
      filterAttribute(node, "type", "alternative")
    )
    constructTitles(altTitles)
  }
  ...
  override def dataProvider(data: Document[NodeSeq]): ZeroToMany[EdmAgent] =
    (data \ "note")
      .map(n => getByAttribute(n, "type", "ownership"))
