...
trait Mapper[T, +E] extends IngestMessageTemplates {
  ...
  /** Logs a bare-IP warning and returns None for optional URL fields (object,
    * preview, hasView). The warn-and-drop pattern is appropriate for optional
    * fields: the record is kept but the broken URL is removed.
    */
  private def filterBareIpUrl(
      url: EdmWebResource,
      providerId: String,
      field: String
  )(implicit collector: MessageCollector[IngestMessage]): Option[EdmWebResource] =
    if (url.uri.hasBareIpHost) {
      collector.add(bareIpUrlMsg(providerId, field, url.uri.value, enforce = false))
      None
    } else Some(url)
  ...
  def validateIsShownAt(
    ...
    values.headOption match {
      case Some(url) =>
        if (url.uri.hasBareIpHost)
          collector.add(bareIpUrlMsg(providerId, "isShownAt", url.uri.value, enforce = true))
        url
      case None =>
  ...
  def validateObject(
    ...
    values.headOption.flatMap(filterBareIpUrl(_, providerId, "object"))
  }
  ...
  def validatePreview(
    ...
    values.headOption.flatMap(filterBareIpUrl(_, providerId, "preview"))
  }

  /** Validates hasView values, filtering out any entries whose URI is a bare IP
    * address. Bare-IP hasView entries are always a provider misconfiguration
    * and are dropped with a warning rather than failing the record.
    *
    * @param values
    *   Seq[EdmWebResource] Values generated by hasView mapping
    * @param providerId
    *   String Local provider identifier
    * @param collector
    *   MessageCollector Ingest message collector
    * @return
    *   Seq[EdmWebResource] hasView entries with bare-IP entries removed
    */
  def validateHasView(
      values: ZeroToMany[EdmWebResource],
      providerId: String
    ...
    values.flatMap(filterBareIpUrl(_, providerId, "hasView"))
...
class XmlMapper extends Mapper[NodeSeq, XmlMapping] {
  override def map(
    ...
    val validatedHasView = validateHasView(
      mapping.hasView(document),
      providerId
    )
    val validatedTitle =
