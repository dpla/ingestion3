...
class LcCsvFileHarvester(
  ...
  /** Loads .csv files
    ...
    *   File to parse
    * @return
    *   Iterator[String] of the CSV lines
    */
  def getInputStream(file: File): Option[Iterator[String]] = {
    file.getName match {
      case csvName
          if csvName.endsWith(
            "csv"
          ) => // FIXME isn't the redundant of the file filter?
    ...
    Option(
      ParsedResult(
        extractor
          .extractString(
            json \\ "id"
          ) // Location of the persistent ID for LC records
          .getOrElse(throw new RuntimeException("Missing ID")),
        compact(render(json))
      )
    )

  /** Parses a row from a CSV file, extracts the data and writes it out
    *
    * @param row
    *   Case class representing extracted items from a CSV row
    * @return
    *   Count of metadata items found.
    */
  def handleRow(row: CsvRow, unixEpoch: Long): Try[Int] = {
    row match {
      case CsvRow(_, None) =>
        Success(0) // no data associated with row
      case CsvRow(Some(_), Some(data)) =>
        Try {
          Try {
            ...
            val json: JValue = parse(
              data
                ...
                .replaceAll(
                  "\"\"",
                  "\""
                ) // replace double-double quote with single double quote
            )
  ...
  /** @param iterator
    *   Iterator[String] read from file
    * @return
    *   Stream[CsvRow] Stream of CsvRows
    */
        ...
        val inputStream: Iterator[String] = getInputStream(inFile)
          .getOrElse(
            throw new IllegalArgumentException(
              s"Couldn't load CSV file - ${inFile.getAbsolutePath}"
            )
          )
