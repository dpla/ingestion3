...
class LocHarvester(
  ...
  /** Fetch all items from LC
    *
    * @param urls
    *   Seq[URL] URLs to fetch
    * @return
    *   List[ApiResponse] List containing ApiErrors and ApiRecords
    */
                ...
                if (recordId.nonEmpty)
                  ...
                  ApiError(
                    ...
                    ApiSource(
                      Map("" -> ""),
                      Option(url.toString),
                      Option(compact(render(json)))
                    )
                  )
              case Failure(parseError) =>
                ApiError(
                  parseError.getStackTrace.mkString("\n\t"),
                  ApiSource(
                    Map("" -> ""),
                    Option(url.toString),
                    Option(response)
  ...
  def buildUrl(params: Map[String, String]): URL =
    new URIBuilder()
      ...
      .setHost("www.loc.gov")
      .setPath(
        s"/collections/${params.getOrElse("collection", throw new RuntimeException("No collection specified"))}"
      )
