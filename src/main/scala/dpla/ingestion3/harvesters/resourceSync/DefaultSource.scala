...
class DefaultSource extends RelationProvider {
  ...
  /**
    ...
    * within this package.  ResourceSyn requests may return additional errors if the
    * given parameters do not conform with ResourceSync rules.
    */
  override def createRelation(sqlContext: SQLContext,
                              parameters: Map[String, String]) : RsRelation = {
  ...
  def getEndpoint(parameters: Map[String, String]): String = {
    ...
    (endpoint, HttpUtils.validateUrl(endpoint.getOrElse(""))) match {
      ...
      case (Some(url), false) => throwValidationException(s"ResourceSync endpoint ${url} is not reachable.")
      ...
      case _ => throwValidationException(s"Unable to validate the ResourceSync endpoint.")
    }
  }
