...
case class RsSource(url: Option[String] = None, text: Option[String] = None)
    extends RsResponse
...
case class RsError(message: String, errorSource: RsSource) extends RsResponse

// A single record from an Resource Sync harvest.
case class RsRecord(id: String, document: String, recordSource: RsSource)
    extends RsResponse
