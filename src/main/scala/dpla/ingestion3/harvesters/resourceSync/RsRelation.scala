...
/**
  ...
  * request to send to the ResourceSync feed (ie. get all records or a change set,
  * get those records by iterating over resourcelist or requesting TAR files via dump).
  * It requests a ResourceSync response via the `RsResponseBuilder`.
  * It constructs a DataFrame from the RS response.
  */
class RsRelation (endpoint: String,
  ...
  override def schema: StructType = {
    val recordType = ScalaReflection.schemaFor[RsRecord].dataType
      .asInstanceOf[StructType]
    val errorType = ScalaReflection.schemaFor[RsError].dataType
      .asInstanceOf[StructType]
