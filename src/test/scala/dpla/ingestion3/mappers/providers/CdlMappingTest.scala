...
class CdlMappingTest extends AnyFlatSpec with BeforeAndAfter {
  ...
  it should "create the correct DPLA URI" in {
    val json = org.json4s.native.JsonMethods.parse("""{ "id": "foo" }""")
  ...
  it should "extract the correct intermediate provider" in {
    val json = org.json4s.native.JsonMethods.parse("""{ "repository_name": [ "foo", "University of Southern California Digital Library" ] }""")
  ...
  it should "extract no intermediate provider" in {
    val json = org.json4s.native.JsonMethods.parse("""{ "repository_name": [ "foo" ] }""")
    val expected = None
    assert(extractor.intermediateProvider(Document(json)) == expected)
  }
}