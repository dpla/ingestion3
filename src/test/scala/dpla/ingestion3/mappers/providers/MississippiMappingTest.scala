...
class MississippiMappingTest extends FlatSpec with BeforeAndAfter {
  ...
  it should "extract the correct original ID" in {
    val expected = Some("https://na04.alma.exlibrisgroup.com/primaws/rest/pub/pnxs/L/991014100609305566")
    assert(extractor.originalId(json) === expected)
  }

  it should "extract the correct previews" in {
    val expected = Seq("http://cdm16631.contentdm.oclc.org/utils/getthumbnail/collection/p16631coll10/id/3729")
  ...
  it should "extract the correct description" in {
    val expected = Seq("Illustration of Native American girl sitting and Native American man on  horse. Forest and teepees")
  ...
  it should "extract the correct identifiers" in {
    val expected = Seq("011491515-1910;32278011491515;Box 85; Folder 1; Piece 7;digital_TIF_archive\\Templeton_master\\Folder_85-1;http://cdm16631.contentdm.oclc.org/cdm/ref/collection/p16631coll10/id/3729;oai:cdm16631.contentdm.oclc.org:p16631coll10/3729")
  ...
  it should "extract the correct contributor" in {
    val expected = Seq("Starmer, William A. (William Austin), b. 1872; Starmer, Frederick S., b. 1879").map(nameOnlyAgent)
    assert(extractor.contributor(json) === expected)
  }

  it should "extract the correct publisher" in {
    val expected = Seq("New York : The Jos. Morris Co.", "Mississippi State University Libraries (electronic version)").map(nameOnlyAgent)
