...
def get_monthly_hubs(month):
    ...
    conf_text = ssm_run(f"cat {CONF_PATH} 2>/dev/null || echo ''", timeout_seconds=30)
    if not conf_text.strip():
        sys.exit(f"[bad] Could not read i3.conf at {CONF_PATH} on EC2.")
...
def build_batch_script(hubs, batch_log):
...
_log "▶ Monthly batch started — ${{#HUBS[@]}} hubs: {hub_list}"
slack_notify ":calendar: *Monthly batch started* — ${{#HUBS[@]}} hubs: {hub_list}"
