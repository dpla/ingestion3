...
0. **Build the JAR:** Run `sbt assembly` from repo root before running ingestion scripts so the fat JAR is current. Scripts (`./scripts/harvest.sh`, `ingest.sh`, `remap.sh`) use `run_entry` in common.sh, which uses the JAR when present or falls back to `sbt runMain` when missing — they do not auto-build the JAR.
